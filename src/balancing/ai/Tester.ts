    ...
    test(enemy: Character, hero: Character) {
        ...
        do {
            ...
            if (
                result.outcome === "hero" &&
                result.rounds > 2 &&
                result.totalHp / result.totalMaxHp < 0.8
            ) {
                console.log(
                    "winner",
                    vector.asTupleString(),
                    result.totalHp / result.totalMaxHp,
                    this.space.vectors.length
                );
                return enemy;
            }
        ...
        console.log(
            this.space.vectors.map((v) => v.vector.asTupleString()).join("\n")
        );
    }
}
